...
def equil_max2p_ss(P, T, yi, Pci, Tci, wi, dij, vsi, mwi, c=1,
                   negative_flash=False, maxiter_flash=500, eps_flash=1e-8, method_stab='ss-newton',
    ...
    if negative_flash: #Негативный флэш, чтобы выйти за физичные значения мольных долей фаз, но остаться в NFW
        ...
        elif method_stab == 'ss-newton':
            is_stable, kvji0, Z = stability_ss_newton(P, T, yi, Pci, Tci, wi, dij, vsi, c, eps1_stab, eps2_stab, maxiter_stab, level, pure_ind, pure_eps, eps_r, eps_u, eps_l)
        else:
...
def equil_ss(P, T, yi, Pci, Tci, wi, dij, vsi, mwi, c=1,
            nphase=3, negative_flash=False, maxiter_flash=500, eps_flash=1e-8, method_stab='ss-newton',
            eps1_stab=1e-10, eps2_stab=1e-8, maxiter_stab=50,
            level=0, pure_ind=0, pure_eps=1e-3,
    ...
    if method_stab == 'ss':
        ...
        is_stable, kvji0, Z = stability_ss_newton(P, T, yji[1], Pci, Tci, wi, dij, vsi, c, eps1_stab, eps2_stab, maxiter_stab, level, pure_ind, pure_eps, eps_r, eps_u, eps_l)
    else:
        raise ValueError('Unknown stability method')
        ...
        for p in range(3, nphase + 1): #Начальные приближения для Np-фазного флеша
            ...
            if gnorm < eps_flash:
                ...
                if method_stab == 'ss':
                    ...
                    is_stable, kvji0, _ = stability_ss_newton(P, T, xi, Pci, Tci, wi, dij, vsi, c, eps1_stab, eps2_stab, maxiter_stab, level, pure_ind, pure_eps, eps_r, eps_u, eps_l)
                else:
                    raise ValueError('Unknown stability method')
                if is_stable:
...
def equil_max2p_ss_newton(P, T, yi, Pci, Tci, wi, dij, vsi, mwi, c=1,
                          negative_flash=False, maxiter_flash=500, eps_flash=1e-16, method_stab='ss-newton',
                          eps1_stab=1e-10, eps2_stab=1e-8, maxiter_stab=50,
                          level=0, pure_ind=0, pure_eps=1e-3,
                          eps_rr=1e-10, maxiter_rr=50,
                          eps_r=0.1, eps_u=1e-4, eps_l=1e-12, eps_v=1e-2):
    ...
    if is_stable:
        ...
        for j, kvi in enumerate(kvji0):
            ...
            # gnorm = np.linalg.norm(gi)
            g2 = gi.dot(gi)
            while g2 > eps_flash and k < maxiter_flash:
                k += 1
                kvi *= np.exp(-gi)
                f1m1 = f1
                f1 = R_R_2p(yi, kvi, eps_rr, maxiter_rr)
                ...
                g2 = gi.dot(gi)
                # print(f'ss: {k = }, {g2 = }')
                if g2 / g2m1 > eps_r and g2 < eps_u and g2 > eps_l and np.abs(f1 - f1m1) < eps_v and f1 > 0 and f1 < 1:
                    break
            if g2 > eps_flash and k < maxiter_flash:
                lnkvi = np.log(kvi)
                f2 = 1 - f1
                n1i = y1i * f1
                n2i = y2i * f2
